...
<div class="container-fluid pt-4 px-4">
    ...
    <div class="container-fluid pt-4 px-4">
        <div class="row bg-nb bg-blue-a rounded align-items-center justify-content-center p-3 mx-1">
           ...
           <h1 align="center"><span id="">หน้าลงเวลาผู้เตรียมสไลด์</span></h1>
           ...
           <div>
                <div class="<?= $isBorder ? "border" : "" ?> ">
                    <label for="p_cross_section_id_job3"  class="form-label">เลือกพนักงานช่วยตัดเนื้อที่ต้องการจะลงเวลา</label>
                    <select name="p_cross_section_id_job3" id="p_cross_section_id_job3" class="form-select"  <?= ( $isCurUserAdmin || ($cur_user_can_manaage_job == 1) ) ? "" : " disabled readonly " ?>>
...
<table class="table table-hover table-striped" id="job3_finish_table" style="width:100%">
                <thead>
                    <tr>
                        <th scope="col">#</th>                
                        <th scope="col">1</th>      
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
            <label for="qty_modal_job3_finish">Qty:</label>
            <input type="text" id="qty_modal_job3_finish" name="qty_modal_job3_finish">
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="save_qty_modal_job3_finish" class="btn btn-primary" data-bs-dismiss="modal">Save changes</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    function colorAdd() {
        ...
        tablee = document.getElementById("job3_finish_table");
    ...
    // datatable
    var table = $('#job3_finish_table').DataTable({
        "ajax": "ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=1m",
        responsive: true,
        dom: 'BPlfrtip',
...
//
        ...
        buttons: [
            ...
            buttons: [{
                ...
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=1m").load();
            ...
            },
            {
                text: '3 เดือนล่าสุด',
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=3m").load();
            ...
            },
            {
                text: '6 เดือนล่าสุด',
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=6m").load();
            ...
            },
            {
                text: '1 ปีล่าสุด',
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=1y").load();
            ...
            },
            {
                text: '2 ปีล่าสุด',
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "&range=2y").load();
            ...
            },
            {
                text: 'ทั้งหมด',
                action: function (e, dt, node, config) {
                    dt.ajax.url("ajax_job3_finish/job3_finish.php?skey=" + skey + "").load();
        ...
        },
        {
            "render": function (data, type, row) {
                ...
                renderdata += '<a id="finish_btn_'+patient_id+'"  onclick="finishJob3ByPatientId('+patient_id+',\''+patient_number+'\',\''+hn+'\');"           class="btn btn-outline-primary btn-sm me-1 edit"><i class="fa-solid fa-marker"></i>Add One</a>';
                renderdata += '<a id="remove_btn_'+patient_id+'"  onclick="removeJob3ByPatientId('+patient_id+',\''+patient_number+'\',\''+hn+'\');"           class="btn btn-outline-primary btn-sm me-1 edit"><i class="fa-solid fa-marker"></i>Remove One</a>';
                if(isCurUserAdmin=='1' || cur_user_can_manaage_job == '1' ){
                    renderdata += '<a id="remove_all_btn_'+patient_id+'"  onclick="removeJob3ByPatientIdAll('+patient_id+',\''+patient_number+'\',\''+hn+'\');"           class="btn btn-outline-primary btn-sm me-1 edit"><i class="fa-solid fa-marker"></i>Remove All</a>';
                }else{
...
function finishJob3ByPatientId(patient_id,patient_number,hn) {
    
    let value = $('#p_cross_section_id_job3 option').filter(':selected').attr('value');
    let txt = $('#p_cross_section_id_job3 option').filter(':selected').text();
    if (value == "0" || value == 0) {
...
//        alert("cur_user_id="+cur_user_id+"job_role_id="+job_role_id+"patient_id="+patient_id);
    ...
    var job_role_id = $('#p_cross_section_id_job3 option').filter(':selected').attr('job_role_id');
//    var patient_id = $('#p_cross_section_id_job3 option').filter(':selected').attr('patient_id');
    var patient_id = patient_id;
//    var patient_number = $('#p_cross_section_id_job3 option').filter(':selected').attr('patient_number');
    var patient_number = patient_number;
    if(hn==null){
        hn='';
    }
    var user_id = $('#p_cross_section_id_job3 option').filter(':selected').attr('user_id');
    var pre_name = $('#p_cross_section_id_job3 option').filter(':selected').attr('pre_name');
    var name = $('#p_cross_section_id_job3 option').filter(':selected').attr('name');
    var lastname = $('#p_cross_section_id_job3 option').filter(':selected').attr('lastname');
    var jobname = $('#p_cross_section_id_job3 option').filter(':selected').attr('jobname');
    var pay = $('#p_cross_section_id_job3 option').filter(':selected').attr('pay');
    var cost_count_per_day = $('#p_cross_section_id_job3 option').filter(':selected').attr('cost_count_per_day');
    var comment = $('#p_cross_section_id_job3 option').filter(':selected').attr('comment');
    ...
    $.ajax({
        ...
        url: 'ajax_job3_finish/job3_finish_btn.php',
        data: {
...
function removeJob3ByPatientId(patient_id,patient_number,hn) {
...
//    var patient_number = $('#p_cross_section_id_job3 option').filter(':selected').attr('patient_number');
    ...
    $.ajax({
        ...
        // make sure you respect the same origin policy with this url:
        // http://en.wikipedia.org/wiki/Same_origin_policy
        url: 'ajax_job3_finish/job3_remove_btn.php',
        data: {
            'job_role_id': job_role_id,
            'patient_id': patient_id,
            'patient_number': patient_number,
...
function removeJob3ByPatientIdAll(patient_id,patient_number,hn) {
...
//    var patient_number = $('#p_cross_section_id_job3 option').filter(':selected').attr('patient_number');
    ...
    $.ajax({
        ...
        // make sure you respect the same origin policy with this url:
        // http://en.wikipedia.org/wiki/Same_origin_policy
        url: 'ajax_job3_finish/job3_remove_all_btn.php',
        data: {
            'job_role_id': job_role_id,
            'patient_id': patient_id,
            'patient_number': patient_number,
...
function setTargetPatient_id(patient_id,sn,hn,qty) {
    ...
    $('#qty_modal_job3_finish').val(cur_qty);
    $('#exampleModalLabel').text(cur_sn);
}

$('#increaseQty').on("click", function (e) {
    let newVal = parseInt( $('#qty_modal_job3_finish').val());
    newVal += 1;
    $('#qty_modal_job3_finish').val(newVal);
...
//save_qty_modal_job3_finish
$('#save_qty_modal_job3_finish').on("click", function (e) {
    let job3qty = parseInt( $('#qty_modal_job3_finish').val());
    ...
    $.ajax({
        ...
        // make sure you respect the same origin policy with this url:
        // http://en.wikipedia.org/wiki/Same_origin_policy
        url: 'ajax_job3_finish/job3_finish_qty.php',
        data: {
...
//            repaintTbljob3(data);
    ...
    span_id.innerText = span_id.textContent = job3qty.toString();
...
<!--<script src="<?= Url::getSubFolder1() ?>/js/job3_finish.js?v0"></script>-->