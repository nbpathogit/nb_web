...
<?php else: ?>
    ...
    <div align=""  class="mb-3">
        ...
        <textarea name="p_rs_specimen" cols="100" rows="5" class="form-control" id="p_rs_specimen" <?= $canEditModePage && $canCurPathoEditAndReleasedResult ? "" : " disabled readonly " ?> ><?= $patient[0]['p_rs_specimen']; ?></textarea>
    </div>

    <div align=""  class="mb-3">
        <label for="p_rs_clinical_diag">CLINICAL DIAGNOSIS</label><br>
        <textarea name="p_rs_clinical_diag" cols="100" rows="5" class="form-control" id="p_rs_clinical_diag"  <?= $canEditModePage && $canCurPathoEditAndReleasedResult ? "" : " disabled readonly " ?>><?= $patient[0]['p_rs_clinical_diag']; ?></textarea>
    </div>

    <div align=""  class="mb-3">
        <label for="p_rs_gross_desc">GROSS DESCRIPTION</label><br>
        <textarea name="p_rs_gross_desc" cols="100" rows="5" class="form-control" id="p_rs_gross_desc" <?= $canEditModePage && $canCurPathoEditAndReleasedResult ? "" : " disabled readonly " ?>><?= $patient[0]['p_rs_gross_desc']; ?></textarea>
    </div>

    <div align=""  class="mb-3">
        <label for="p_rs_microscopic_desc">MICROSCOPIC DESCRIPTION </label><br>
        <textarea name="p_rs_microscopic_desc" cols="100"  rows="5" class="form-control" id="p_rs_microscopic_desc" <?= $canEditModePage && $canCurPathoEditAndReleasedResult ? "" : " disabled readonly " ?>><?= $patient[0]['p_rs_microscopic_desc']; ?></textarea>
    </div>
...
<div class="row <?= $isBorder ? "border" : "" ?>">
    <div class="col-xl-4 col-md-6 <?= $isBorder ? "border" : "" ?> ">
        ...
        <select name="ppathologist_id" id="ppathologist_id" class="form-select" <?= $canEditModePage && $canCurPathoEditAndReleasedResult ? "" : " disabled readonly " ?> >
