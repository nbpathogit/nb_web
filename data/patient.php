...
if (
    ...
    if (
        ...
        (strpos(
            ...
            $_SERVER["HTTP_HOST"] . Url::getSubFolder1() . "/patient.php",
        ) ||
            strpos(
                $_SERVER["HTTP_REFERER"],
                $_SERVER["HTTP_HOST"] .
                    Url::getSubFolder1() .
                    "/patient_monitor_8000.php",
            ) ||
            strpos(
                $_SERVER["HTTP_REFERER"],
                $_SERVER["HTTP_HOST"] .
...
if ($auth) {
    ...
    if (isset($_REQUEST["range"])) {
        if (!is_numeric($_REQUEST["range"])) {
            ...
            } elseif ($_REQUEST["range"] == "2m") {
                $dateTime = new DateTime("-2 Months");
            } elseif ($_REQUEST["range"] == "3m") {
                $dateTime = new DateTime("-3 Months");
            } elseif ($_REQUEST["range"] == "6m") {
                $dateTime = new DateTime("-6 Months");
            } elseif ($_REQUEST["range"] == "1y") {
                $dateTime = new DateTime("-1 Years");
            } elseif ($_REQUEST["range"] == "2y") {
    ...
    foreach ($patientLists as $patient) {
        if ($patient["pid"]) {
            //phospital_num
            if (
                $_SESSION["user"]->ugroup_id == "5000" ||
                $_SESSION["user"]->ugroup_id == "5100"
            ) {
                if (
                    $_SESSION["user"]->uhospital_id == $patient["phospital_id"]
                ) {
                    // Filter out IN and ZN p_sn_type for customers
                    if (
                        $patient["p_sn_type"] != "IN" &&
                        $patient["p_sn_type"] != "ZN"
                    ) {
                        // Select only Owner Hospital
                        $data[] = [
                            $patient["pid"], //0
                            $patient["p_sn_type"], //1
                            $patient["p_pnum"], //2
